...
var holes_elements = [Vector2i(0,0), Vector2i(1,0), Vector2i(2,0)]
var cactus_elements = [Vector2i(0,1), Vector2i(2,2), Vector2i(6,0),#cactus
					   Vector2i(9,0), Vector2(12,0), Vector2(15,0),
					   Vector2i(20,2), Vector2(22,1), Vector2(0,5)]
...
func bloque_cavado(cell: Vector2i) -> void:
	excavacion_tile_map_layer.set_cell(cell, atlas_id_bloques_alrededor, holes_elements.pick_random())
	disabled_dig[cell] = 0
		...
		for x in range(map_width):
			...
			if mapa[y][x] == 0:
				...
				if noise_tree_val > 0.915  and noise_lake_val < 0.43 :
					disabled_dig[pos] = 0
					enviroment_tile_map_layer.set_cell(pos/2, atlas_id_cactus, cactus_elements.pick_random())
